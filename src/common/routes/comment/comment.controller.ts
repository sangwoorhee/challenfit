...
export class CommentController {
  ...
  @ApiOperation({
    summary: '댓글 조회',
    description: 'GET: /comment/:comment_idx',
  })
  async getComment(
    @Param('comment_idx') commentIdx: string,
  ...
  @ApiOperation({
    summary: '댓글 목록 조회',
    description: 'GET: /comment/workoutcert/:workoutcert_idx',
  })
  async getCommentsByWorkoutCert(
    @Param('workoutcert_idx') workoutCertIdx: string,
  ) {
    const comments =
      await this.commentService.getCommentsByWorkoutCert(workoutCertIdx);
