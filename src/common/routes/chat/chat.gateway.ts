// src/common/routes/chat/chat.gateway.ts
import {
  WebSocketGateway,
  SubscribeMessage,
  MessageBody,
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleJoinRoom(
    ...
    try {
      ...
      // 채팅 내역 전송 (페이지네이션)
      const messages = await this.chatService.getChatHistory(challengeRoomIdx, 1, 50);
      ...
      // 현재 접속자 목록 전송
      const onlineUsers = await this.chatService.getRoomOnlineUsers(challengeRoomIdx);
  ...
  async handleDeleteMessage(
    ...
    try {
      const deletedMessage = await this.chatService.deleteMessage(messageIdx, userIdx);
  ...
  async handleTyping(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: { challengeRoomIdx: string; userIdx: string; isTyping: boolean },
  ) {
